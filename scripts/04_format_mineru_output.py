...
def main() -> None:
    ...
    parser.add_argument("--corpus-dir", help="Canonical corpus workspace.")
    parser.add_argument("--mineru-raw", help="Legacy explicit MinerU output path.")
    parser.add_argument("--out-dir", help="Legacy explicit normalized output path.")
    ...
    elif args.mineru_raw and args.out_dir:
        raw_dir = Path(args.mineru_raw).expanduser().resolve()
        papers_dir = Path(args.out_dir).expanduser().resolve()
        manifest_path = raw_dir.parent / "manifests" / "documents.jsonl"
        canonical_manifest_path = manifest_path
    else:
        parser.error("use --corpus-dir, or both --mineru-raw and --out-dir")
